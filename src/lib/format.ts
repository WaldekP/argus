/**
 * Formatowanie danych do wyświetlenia. Konwencja: polskie separatory,
 * brak danych zawsze jako "brak danych", nigdy jako zero ani pusty string.
 */

/** Data z bazy (RRRR-MM-DD) na polski zapis DD.MM.RRRR. */
...
/** Odmiana słowa "rok" przez liczbę: 1 rok, 2 lata, 5 lat. */
export function yearsLabel(years: number): string {
  ...
  const last = years % 10;
  const plural = last >= 2 && last <= 4 && !(lastTwo >= 12 && lastTwo <= 14);
  return `${years} ${plural ? 'lata' : 'lat'}`;
}
